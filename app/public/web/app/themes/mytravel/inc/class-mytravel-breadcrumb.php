...
class Mytravel_Breadcrumb {
	...
	public function get_breadcrumb() {
		return apply_filters( 'mytravel_get_breadcrumb', $this->crumbs, $this );
	}
	...
	protected function add_crumbs_404() {
		$this->add_crumb( esc_html__( 'Error 404', 'mytravel' ) );
	}
	 ...
	 * @param string $permalink Post permalink.
	 */
	protected function add_crumb_single( $post_id = 0, $permalink = '' ) {
		if ( ! $post_id ) {
			global $post;
		} else {
			$post = get_post( $post_id ); // WPCS: override ok.
		...
		if ( 'product' === get_post_type( $post ) ) {
			...
			$terms = wc_get_product_terms(
				...
				'product_cat',
				apply_filters(
					'mytravel_breadcrumb_product_terms_args',
					array(
						'orderby' => 'parent',
						'order'   => 'DESC',
					)
			...
			if ( $terms ) {
				$main_term = apply_filters( 'mytravel_breadcrumb_main_term', $terms[0], $terms );
	...
	protected function add_crumbs_product_tag() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Products tagged &ldquo;%s&rdquo;', 'mytravel' ), $current_term->name ), get_term_link( $current_term, 'product_tag' ) );
	}
	...
	protected function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Posts tagged &ldquo;%s&rdquo;', 'mytravel' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	protected function add_crumbs_author() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Author: %s', 'mytravel' ), $userdata->display_name ) );
	}
	...
	protected function endpoint_trail() {
		$endpoint       = ( mytravel_is_woocommerce_activated() && is_wc_endpoint_url() ) ? WC()->query->get_current_endpoint() : '';
	...
	protected function search_trail() {
		if ( is_search() ) {
			...
			$this->add_crumb( sprintf( esc_html__( 'Search results for &ldquo;%s&rdquo;', 'mytravel' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	protected function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			...
			$this->add_crumb( sprintf( esc_html__( 'Page %d', 'mytravel' ), get_query_var( 'paged' ) ) );
		}
	}
}
